          ...
          <motion.div initial={false} whileInView={reducedMotion === false ? { opacity: [0.7, 1], y: [14, 0] } : undefined} viewport={{ once: true, amount: 0.15 }} transition={{ duration: 0.5, delay: 0.08 }} className="mx-auto max-w-[46rem] border-t border-ink/20">
            {faqs.map((faq, i) => (
              ...
              >
                ...
                >
                  <span aria-hidden="true" className="self-start pt-1 text-xs font-medium tabular-nums text-cocoa">
                  ...
                  <span
                    className="flex size-8 shrink-0 items-center justify-center rounded-full border border-ink/20 transition-colors duration-200 group-open:border-ink group-open:bg-peach motion-reduce:transition-none sm:size-10"
                    aria-hidden="true"
                  >
                ...
                >
                  <div className="min-h-0 overflow-hidden pb-7 pl-10 pr-4 sm:pl-16 sm:pr-20">
                    <p className="max-w-xl text-sm leading-7 text-cocoa sm:text-base">
                      {faq.answer}
                    </p>
                  </div>
                </div>
