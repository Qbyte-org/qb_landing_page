  ...
  useGSAP(
    () => {
      ...
      const headings = gsap.utils
        .toArray<HTMLElement>("main section h2", shell)
        .filter((heading) => !heading.closest("[data-skip-section-title-motion]"));
      const sectionHeaders = Array.from(
        new Set(
          headings.map(
            (heading) =>
              heading.closest<HTMLElement>("[data-section-motion-header]") ??
              heading,
          ),
        ),
      );
      ...
      if (reducedMotion) {
        gsap.set([...headings, ...sectionHeaders], {
          autoAlpha: 1,
          clearProps: "transform,filter,clipPath",
        });
        return;
